...
Console::Console(const PixelColor& fg_color, const PixelColor& bg_color)
  ...
  Vector2D<int> size=ScreenSize();
  kColumns=size.x/8>1000?1000:size.x/8;
  kRows=size.y/16>1000?1000:size.y/16;
}

void Console::PutString(const char* s) {
  WriteString(*writer_, Vector2D<int>{cursor_column_*8, 16 * cursor_row_}, "_", bg_color_);
  while (*s) {
    if (*s == '\n') {
      Newline();
  ...
  WriteString(*writer_, Vector2D<int>{cursor_column_*8, 16 * cursor_row_}, "_", fg_color_);
  if (layer_manager) {
